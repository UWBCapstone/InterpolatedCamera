Shader "Custom/InterpolatedCameraShader_BoxFilter2" {
	Properties
	{
...
/*
		...
		float2 uv7 = uv;
		float4 tex7 = tex2D(_Tex7, uv7);


		// Calculate the weighted values for the textures

		float inTex0 = step(0, uv0.x) * step(0, uv0.y) * step(uv0.x, 1) * step(uv0.y, 1);
		float inTex1 = step(0, uv1.x) * step(0, uv1.y) * step(uv1.x, 1) * step(uv1.y, 1);
		float inTex2 = step(0, uv2.x) * step(0, uv2.y) * step(uv2.x, 1) * step(uv2.y, 1);
		float inTex3 = step(0, uv3.x) * step(0, uv3.y) * step(uv3.x, 1) * step(uv3.y, 1);
		float inTex4 = step(0, uv4.x) * step(0, uv4.y) * step(uv4.x, 1) * step(uv4.y, 1);
		float inTex5 = step(0, uv5.x) * step(0, uv5.y) * step(uv5.x, 1) * step(uv5.y, 1);
		float inTex6 = step(0, uv6.x) * step(0, uv6.y) * step(uv6.x, 1) * step(uv6.y, 1);
		float inTex7 = step(0, uv7.x) * step(0, uv7.y) * step(uv7.x, 1) * step(uv7.y, 1);

		float2 w_0 = float2(2*(0.5 - abs(0.5-uv0.x)), 2*(0.5 - abs(0.5-uv0.y)));
		float2 w_1 = float2(2*(0.5 - abs(0.5-uv1.x)), 2*(0.5 - abs(0.5-uv1.y)));
		float2 w_2 = float2(2*(0.5 - abs(0.5-uv2.x)), 2*(0.5 - abs(0.5-uv2.y)));
		float2 w_3 = float2(2*(0.5 - abs(0.5-uv3.x)), 2*(0.5 - abs(0.5-uv3.y)));
		float2 w_4 = float2(2*(0.5 - abs(0.5-uv4.x)), 2*(0.5 - abs(0.5-uv4.y)));
		float2 w_5 = float2(2*(0.5 - abs(0.5-uv5.x)), 2*(0.5 - abs(0.5-uv5.y)));
		float2 w_6 = float2(2*(0.5 - abs(0.5-uv6.x)), 2*(0.5 - abs(0.5-uv6.y)));
		float2 w_7 = float2(2*(0.5 - abs(0.5-uv7.x)), 2*(0.5 - abs(0.5-uv7.y)));

		float w0 = step(0.01, inTex0) * w_0.x * w_0.y;
		float w1 = step(0.01, inTex1) * w_1.x * w_1.y;
		float w2 = step(0.01, inTex2) * w_2.x * w_2.y;
		float w3 = step(0.01, inTex3) * w_3.x * w_3.y;
		float w4 = step(0.01, inTex4) * w_4.x * w_4.y;
		float w5 = step(0.01, inTex5) * w_5.x * w_5.y;
		float w6 = step(0.01, inTex6) * w_6.x * w_6.y;
		float w7 = step(0.01, inTex7) * w_7.x * w_7.y;

		float wTotal =  w0 + w1 + w2 + w3 + w4 + w5 + w6 + w7;
		float4 texTotal = w0 * tex0 + w1 * tex1 + w2 * tex2 + w3 * tex3 + w4 * tex4 + w5 * tex5 + w6 * tex6 + w7 * tex7;
		fixed4 c = texTotal / wTotal;
